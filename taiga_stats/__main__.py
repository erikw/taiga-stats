...
def main():
    ...
    cnf_url, cnf_auth_token, cnf_project_id, cnf_tag, cnf_output_path, \
        cnf_target_date, cnf_target_layer, cnf_status_ids, cnf_annotations_off, cnf_print_tags, cnf_print_points = config.read_config()
    ...
    args['target_layer'] = args['target_date_and_layer'][1] \
        if 'target_date_and_layer' in args and args['target_date_and_layer'] else cnf_target_layer if cnf_target_layer else None
    ...
    args['annotations_off'] = args['annotations_off'] \
        if 'annotations_off' in args and args['annotations_off'] is not None else cnf_annotations_off if cnf_annotations_off is not None else False
    args['print_tags'] = args['print_tags'] \
        if 'print_tags' in args and args['print_tags'] is not None and args['print_tags'] else cnf_print_tags if cnf_print_tags is not None else False
    args['print_points'] = args['print_points'] \
